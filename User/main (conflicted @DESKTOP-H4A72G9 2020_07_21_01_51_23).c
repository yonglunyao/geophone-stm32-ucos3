...
Packet_connectData data={"000004","dlut",RESET_NO,STATUS_OK,0,0,100};//数据上报结构体
...
//	OLED_ShowString(0,3,(u8*)"    WELCOME!    ");
	...
	Set_Mode();
	delay_ms(500);
	IWDG_Init(5,625); //看门狗初始化.在file task中喂狗
	while(1)
	{
		delay_ms(500);
...
#if OLED_ENABLE
	if(sent_files_num>999) sent_files_num=0;//发送个数超过1000后重新计数
	if(local_files_num>99)
		sprintf((char*)oled_buf,"Wait:99 Sent:%03d",sent_files_num);
	else
...
start:
		...
		res=sim7600_send_cmd(ftppath,(u8*)"211",5000);//发送前先删除，用于删除上一次复位后的残余文件,此处仅检查返回是否为211，
																										//若文件存在会返回OK,在下面进行判断是哪一种情况
																										//如果此时返回OK，需要等待10000倒计时为0才会继续执行
...
//	OS_TaskSuspend((OS_TCB*)&FtpTaskTCB,&err);		//挂起FTP任务
	...
	DIR dir;
	FRESULT fres = 1;
	u8* sdrbuff=mymalloc(SRAMIN,9010);//
	...
	if(s != USB_INT_SUCCESS)
	{
		printf("U盘初始化失败\r\n");
		//自动切换到FTP任务
		myfree(SRAMIN,sdrbuff);
#if ENABLE_OLED
		OLED_ShowString(0,3,(u8*)"   USB error    ");
#endif
		OSSchedLock(&err);//调度器加锁
		OSTaskDel((OS_TCB*)&UsbTaskTCB,&err);		//删除USB任务
...
//	CH376FileClose(TRUE);
	while(1)
	{
...
#if ENABLE_OLED
		OLED_ShowString(0,3,(u8*)"   NO U Disk    ");
#endif
			times++;
			OSTimeDlyHMSM(0,0,0,200,OS_OPT_TIME_HMSM_STRICT,&err);
		}
		...
		while(s!=0x14)
		{
			...
			if(s!=0x14)printf("UFR:%02x\r\n",s);
		}
		printf("U Disk Create:%s\r\n",fn);
		...
		if (fileinfo.fsize == 30)
		{
			...
			while (fres)
			{
				...
				if(sent_files_num>999) sent_files_num=0;//发送个数超过1000后重新计数
				if(local_files_num>99)
					sprintf((char*)oled_buf,"Wait:99 Copy:%03d",sent_files_num);
				else
		...
		//写入顺序:先按扇区写入87*20*512=890880字节，余9120字节
		//9120字节先按扇入写入17*512=8704字节，余416字节
		//416字节直接按字节写入方式写入
		//共计900000字节
		else if (fileinfo.fsize == 900000)
		{
			...
			{
				OSMutexPend(&sdmutex,0,OS_OPT_PEND_BLOCKING,0,&err); //等待sdmutex信号量
				//从SD卡文件中读取数据放到缓冲区，一次读32，分320次读完，共10k,然后一次性写入U盘
				for(u16 j=0;j<300;j++)
				{
				...
				while(s!=0x14)
				{
					s=CH376ByteWrite((u8*)sdrbuff,9000, NULL); //向U盘写入文件，考虑是否加入判断返回值过程
					if(s!=0x14)printf("+UWR:%02x\r\n",s);
				}
				//显示发送完成百分比，Add By Yao
				finish_size +=9000;
...
//				for(u16 j=0;j<285;j++)
//				{
//					fres = f_read(&sdfp, &sdrbuff[j*32], 32, &br);
//				}
//				OSMutexPost(&sdmutex,OS_OPT_POST_FIFO,&err);  //释放sdmutux信号量，且就绪阻塞态进程
//				s=1;
//				while(s!=0x14)//先写入17扇区，共计8704字节，剩余9120-17*512=416
//				{
//					s=CH376SecWrite((u8*)sdrbuff,17, NULL); //向U盘写入文件，考虑是否加入判断返回值过程
//					if(s!=0x14)printf("+UWR:%02x\r\n",s);
//				}
//				while(s!=0x14)//最后416字节按字节写入
//				{
//					s=CH376ByteWrite((u8*)&sdrbuff[17*512],416, NULL); //向U盘写入文件，考虑是否加入判断返回值过程
//					if(s!=0x14)printf("+UWR:%02x\r\n",s);
//				}
