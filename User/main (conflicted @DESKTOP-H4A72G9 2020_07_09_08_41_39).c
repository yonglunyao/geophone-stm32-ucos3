...
//任务堆栈大小	
#define READ_STK_SIZE 		150 //原值96 Edit By Yao
//任务控制块
OS_TCB ReadTaskTCB;
//任务堆栈	
...
//任务堆栈大小	
#define GPS_STK_SIZE 		128 //原值64 Edit By Yao
//任务控制块
OS_TCB GpsTaskTCB;
//任务堆栈	
...
//任务堆栈大小	
#define FILE_STK_SIZE 		500 //原值330 Edit By Yao
//任务控制块
OS_TCB FileTaskTCB;
//任务堆栈	
...
//任务堆栈大小	
#define FTP_STK_SIZE 		500 //原值330 Edit By Yao
//任务控制块
OS_TCB FtpTaskTCB;
//任务堆栈	
...
//任务堆栈大小	
#define MONITOR_STK_SIZE 		150 //原值96 Edit By Yao
//任务控制块
...
{  	
	...
	sprintf((char*)buf,"Station %d",NUMBER);
	OLED_Clear();
	OLED_ShowString(25,0,(u8*)buf);
	OLED_ShowString(30,3,(u8*)"WELCOME!");
...
//		printf("读取芯片ID\r\n");
		if (id0 != 3 || id1 != 3 || id2 != 3)
		{
			...
			if(id0 != 3)	OLED_ShowString(0,3,(u8*)"   AD:1 error   ");
			if(id1 != 3)	OLED_ShowString(0,3,(u8*)"   AD:2 error   ");
			if(id2 != 3)	OLED_ShowString(0,3,(u8*)"   AD:3 error   ");
		}
		else
		{
//			printf("Ok, ASD1256 Chip ID = 0x%02X & 0x%02X & 0x%02X\r\n", id0, id1, id2);	//EDit By Yao
			break;
		}
	}
//***************************************************************************
...
//	printf("start task\r\n\r\n"); //Edit By Yao
  ...
  OSSemCreate((OS_SEM      *)&read,    //指向信号量变量的指针
	...
	OS_TaskSuspend((OS_TCB*)&ReadTaskTCB,&err);		//挂起开始任
	OS_TaskSuspend((OS_TCB*)&FileTaskTCB,&err);		//挂起开始任
	OS_TaskSuspend((OS_TCB*)&FtpTaskTCB,&err);		//挂起开始任
...
//TIM2_Init(10000-1, 8500-1); //用于检测GPS状态
	...
	printf("等待GPS定位完成...\r\n\r\n");	//Add By Yao
	OLED_ShowString(0,3,(u8*)" GPS locating...");
 	USART3_RX_STA=0;	
...
void file_task(void *p_arg)
{
	...
	{	
		printf("SD卡错误！\r\n");
		OLED_ShowString(0,3,(u8*)"  SD Card Error ");
...
//	IWDG_Init(5,625); //看门狗初始化
	IWDG_Init(5,625); //增加看门狗时间，用于解决启动时反复复位的问题，Edit By Yao
	while(1)
	{
...
//		SendUsart2TXBUF(1);
//		USART2_RX_STA=0;
		res = sim7600_send_cmd((u8 *)ftppath,(u8 *)"BEGIN",5000);	//问题：启动后第一次发送数据文件会超时，发送头文件正常，SIM7600复位后正常
		/*超时复位，此处若FTP服务器迟迟不给回应会引起复位	Add By Yao*/
		if(res==1)
		{
