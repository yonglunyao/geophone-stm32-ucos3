...
//			res = sim7600_send_cmd((u8 *)ftppath,(u8 *)"BEGIN",6000);	//Edit By Yao
		...
		while(res)
		{
			...
			OSTimeDlyHMSM(0,0,0,100,OS_OPT_TIME_HMSM_STRICT,&err);
//			res = sim7600_send_cmd((u8 *)ftppath,(u8 *)"BEGIN",6000);	
			res = sim7600_send_cmd((u8 *)ftppath,(u8 *)"BEGIN",60000);//Edit By Yao
			printf("ftpput:%d   %s\r\n", res, USART2_RX_BUF);
			//等待超时强制硬件复位SIM7600 Add By Yao
			if(res==1)
			{
				SIM7600_Reset();
				errtimes=3;//标记为3，强制检测SIM7600
				printf("等待ftp回应超时,SIM7600复位！\r\n\r\n");	
		...
		if (fileinfo.fsize == 30)
		{
			...
			{
				//对文件中本身存在的与结束符相同的0x03和0x1A进行转义
				//FTP AT+CFTPPUT指令传输文件以前一位无0x03的0x1A为结束符（0x03 0x1A连续两个字符不是结束符）
		...
		{
			//
			u32 finish_size=0;//用于计算完成字节数
			//一次发30，分30000次发送 Add By Yao
			for (i = 0; i < 30000; ++i)
			{
				OSMutexPend(&sdmutex,0,OS_OPT_PEND_BLOCKING,0,&err); //等待sdmutex信号量
				...
				finish_size+=30;	//Add By Yao
				//显示发送完成比例，每5%为一级 Add By Yao
