...
UINT32	CH376ReadVar32( UINT8 var )
{
	u8 a0, a1, a2, a3;
	u32 res;
	xWriteCH376Cmd(CMD14_READ_VAR32); // 设备USB工作模式
	xWriteCH376Data(var); //06H=已启用的主机方式并且自动产生SOF包
	a0 = xReadCH376Data();	
	a1 = xReadCH376Data();	
	a2 = xReadCH376Data();	
	a3 = xReadCH376Data();
	res = a0 | (UINT16)a1 << 8 | (UINT32)a2 << 16 | (UINT32)a3 << 24;
	return res;
}
...
{
	u8 res;
	xWriteCH376Cmd(mCmd);    // 测试单片机与CH376之间的通讯接口
	res = xReadCH376Data();
	return( res );
}
...
{
	xWriteCH376Cmd( CMD0H_DISK_CONNECT ) ;								/* 检查磁盘是否连接 */
	u8 res = xReadCH376Data();
	printf("DiskConnect:0x%02X\r\n", res);
	return res;
}
...
{
	xWriteCH376Cmd( CMD0H_DISK_MOUNT );									/* 初始化磁盘并测试磁盘是否就绪 */
	u8 res = xReadCH376Data();
	printf("DiskMount:0x%02X\r\n", res);
	return res;
}
...
* 描      述      : 打开多级目录下的文件或者目录的上级目录,支持多级目录路径,支持路径分隔符,路径长度不超过255个字符

* 输      入      : PUINT8 path:指向路径缓冲区.*					
*					          UINT8 StopName:指向最后一级文件名或者目录名*					
...
* 描      述      : 打开多级目录下的文件或者目录(文件夹),支持多级目录路径,支持路径分隔符,路径长度不超过255个字符*
* 输      入      : PUINT8 path:指向路径缓冲区.*					
* 返      回      : 返回最后一级文件名或者目录名的字节偏移.
*******************************************************************************/
...
* 描      述      : 新建多级目录下的文件并打开,支持多级目录路径,支持路径分隔符,路径长度不超过255个字符.
...
/*******************************************************************************
* 函  数  名      : CH376DirCreatePath
* 描      述      : 新建多级目录下的文件夹,支持多级目录路径,支持路径分隔符,路径长度不超过255个字符
*										（一次只能创建一个子文件夹）
* 输      入      : PUINT8 path:	指向路径缓冲区.
* 返      回      : 操作状态.
*******************************************************************************/
...
UINT8	CH376ByteRead( PUINT8 buf, UINT16 ReqCount, PUINT16 RealCount )
{
	...
	TxBuf[0] = 0x57;
	TxBuf[1] = 0xab;
	TxBuf[2] = CMD2H_BYTE_READ;
	TxBuf[3] = (u8)ReqCount;
	TxBuf[4] = (u8)(ReqCount>>8);
	SendUsart6TXBUF(5);
	s = l = xReadCH376Data();
	printf("xReadCH376Data return: 0x%02X\r\n", s);
	if ( RealCount ) 
	{
	    *RealCount = 0;
	}
	...
	while ( 1 ) 
	{
		...
		if ( s == USB_INT_DISK_READ )                                                   // 请求数据读出 
		{
			...
			l = CH376SendCmdWaitInt( CMD0H_BYTE_RD_GO );                                         // 继续读 
			printf("CH376ReadBlock return: 0x%02X\r\n", s);
			printf("CMD0H_BYTE_RD_GO return: 0x%02X\r\n", l);
			xEndCH376Cmd( );
			buf += s;
			if ( RealCount ) 
			{
...
* 返      回      : 中断状态.
*******************************************************************************/
u8	CH376Write(PUINT8 buf, u8 ReqCount)
{
	u8 i, s = USB_INT_DISK_WRITE;
	//printf("进入函数\r\n");
	USART6_RX_REC_ATCOMMAD = 1;
	TxBuf[0] = 0x57;
	TxBuf[1] = 0xab;
	TxBuf[2] = CMD2H_BYTE_WRITE;
	TxBuf[3] = (u8)ReqCount;
	TxBuf[4] = (u8)(ReqCount>>8);
	SendUsart6TXBUF(5);
	//printf("命令1发送完成\r\n");
	...
	if ( s == USB_INT_DISK_WRITE ) 
	{
		for (i = 0; i < ReqCount; ++i)
			TxBuf[3 + i] = buf[i];
		TxBuf[2] = CMD01_WR_REQ_DATA;
		TxBuf[3 + ReqCount + 0] = 0x57;
		TxBuf[3 + ReqCount + 1] = 0xab;
		TxBuf[3 + ReqCount + 2] = CMD0H_BYTE_WR_GO;
		//printf("命令2开始发送\r\n");
		USART6_RX_REC_ATCOMMAD = 1;
		SendUsart6TXBUF(3 + ReqCount + 3);
		//printf("命令2发送完成\r\n");
		while(1) 
		{
			if(USART6_RX_STA&0X8000)	
	...
	if ( RealCount ) 
  {
	...
	while ( 1 ) 
	{
		...
		if ( s == USB_INT_DISK_WRITE ) 
		{
			...
			l = CH376SendCmdWaitInt( CMD0H_BYTE_WR_GO );
			buf += s;
