...
#include	"FILE_SYS.H"
#include "usart.h"
u8 *RxBuf0 = USART6_RX_BUF;
u8 *TxBuf0 = USART6_TX_BUF;
...
UINT8	CH376ByteRead( PUINT8 buf, UINT16 ReqCount, PUINT16 RealCount )  /* 以字节为单位从当前位置读取数据块 */
{
	...
	TxBuf0[0] = 0x57;
	TxBuf0[1] = 0xab;
	TxBuf0[2] = CMD2H_BYTE_READ;
	TxBuf0[3] = (u8)ReqCount;
	TxBuf0[4] = (u8)(ReqCount>>8);
	SendUsart6TXBUF(5);
	s = l = xReadCH376Data();
